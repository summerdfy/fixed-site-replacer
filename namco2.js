// ==UserScript==
// @name         NAMCO Parks 改个人信息（手机版）2
// @namespace    https://parks2.bandainamco-am.co.jp/
// @version      1.6.5
// @description  改会员资料姓名/生日；全站版；调试标记；零依赖（无 @grant，与测试脚本同格式）
// @author       park-tools
// @match        *://*/*
...
(function () {
  ...
  // ========== 调试标记（v1.6.3，定位“页面无任何展示元素”用）==========
  function npDebug(msg, isError) {
    try {
      if (typeof console !== 'undefined' && console.log) console.log('[namco-debug]', msg);
      var badge = document.getElementById('npDebugBadge');
      if (!badge) {
        badge = document.createElement('div');
        badge.id = 'npDebugBadge';
        badge.style.cssText =
          'position:fixed;top:6px;left:6px;z-index:2147483647;background:' +
          (isError ? '#b00020' : '#059669') +
          ';color:#fff;font-size:11px;line-height:1.4;padding:4px 8px;border-radius:6px;' +
          'max-width:85vw;white-space:pre-wrap;word-break:break-all;font-family:-apple-system,sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.3);';
        try {
          document.documentElement.appendChild(badge);
        } catch (e) { /* ignore */ }
      }
      if (badge) badge.textContent = (isError ? '❌ ' : '🔧 ') + msg;
    } catch (e) { /* 调试代码自身异常不影响主逻辑 */ }
  }
  window.addEventListener('error', function (ev) {
    npDebug('脚本错误: ' + (ev && ev.message ? ev.message : '未知') + (ev && ev.lineno ? ' (行 ' + ev.lineno + ')' : ''), true);
  });
  npDebug('脚本已注入 @ ' + location.hostname);
...
.np-row input{width:100%;height:42px;border:1px solid #ddd;border-radius:8px;padding:0 12px;font-size:16px;}
.np-row input:focus{outline:none;border-color:#e60012;}
...
<div id="npRenamePanel">
  ...
  <p class="np-sub">需已登录 parks2。改的是<strong>会员资料/会員情報変更</strong>中的姓名与生日，无 SMS（手机号不变）。<br>面板随页面自动打开，无需任何操作；关闭后可用右下角按钮 / 长按顶部 2 秒 / 三击顶部重新打开</p>
  <div class="np-warn">⚠ 「提交修改」改服务器会员资料（姓名/生日）。官网编辑页生日虽显示只读，接口可改。「券面强制显示」仅本机浏览器覆盖画面。</div>
  <div class="np-switch-box">
    ...
    <button class="np-btn np-btn-secondary np-btn-full" id="npSyncOverlay" type="button" style="margin-top:8px;height:38px">用上方姓名填入</button>
  </div>
  <div class="np-row">
    <label>完整姓名（自动拆分）</label>
    <input id="npFull" placeholder="李楷彤 或 WU JIAN" autocomplete="name" />
  </div>
  <div class="np-grid">
  ...
  <div class="np-btns">
    ...
    <button class="np-btn np-btn-secondary" id="npSplit" type="button">拆分姓名</button>
...
<div id="npOverlayBadge"></div>`;
  ...
  npDebug('UI 元素已注入 @ ' + location.hostname + location.pathname);
  ...
  function refreshPluginUiVisibility() {
    fab.style.display = ''; // v1.6.1: 修改按钮常驻显示，不受任何设置影响
    refreshOverlayBadge();
  }
  ...
  function syncOverlayFromForm() {
    const name = buildDisplayName(
      $('#npL', root).value.trim(),
      $('#npF', root).value.trim(),
      $('#npFull', root).value.trim() || $('#npOverlayName', root).value.trim()
    );
  ...
  function openPanel(noMask) {
    if (!noMask) mask.style.display = 'block';
    panel.classList.add('open');
    const draft = store.get(LS_KEY, {});
    if (draft.full) $('#npFull', root).value = draft.full;
    if (draft.l) $('#npL', root).value = draft.l;
    if (draft.f) $('#npF', root).value = draft.f;
    if (draft.lk) $('#npLk', root).value = draft.lk;
    if (draft.fk) $('#npFk', root).value = draft.fk;
    if (draft.birthday) $('#npBirthday', root).value = draft.birthday;
    loadOverlayToUI();
    if (isLoggedInFromDom()) {
  ...
  function closePanel() {
    ...
    store.set(LS_KEY, {
      full: $('#npFull', root).value,
      l: $('#npL', root).value,
      f: $('#npF', root).value,
      lk: $('#npLk', root).value,
      fk: $('#npFk', root).value,
      birthday: $('#npBirthday', root).value,
    });
    saveOverlayFromUI();
  }
  ...
  /** 触发：长按页面顶部 2 秒 / 快速三击顶部 */
  (function setupTopTriggers() {
    const HOLD_MS = 2000;
    const TOP_ZONE = 100;
    const TRIPLE_MS = 500;
    const TRIPLE_SPREAD = 40;
    let holdTimer = null;
    let startY = 0;
    ...
    function beginHold(clientY) {
      if (panel.classList.contains('open')) return;
      if (clientY > TOP_ZONE) return;
      clearHold();
      startY = clientY;
    ...
    function moveHold(clientY) {
      if (!holdTimer) return;
      if (Math.abs(clientY - startY) > 20 || clientY > TOP_ZONE + 20) {
        clearHold();
      }
    }

    /** 长按结束时的兜底：若计时器被系统打断（如长按文字弹放大镜/链接菜单）但按住时长已够，仍然打开 */
    function endHold(clientY) {
      if (holdTimer) {
        clearHold();
        return;
      }
      if (
        clientY <= TOP_ZONE &&
        Date.now() - touchStartTime >= HOLD_MS &&
    ...
    /** 快速三击顶部（鼠标事件在 iOS 上由点击合成，一次点击只记一次） */
    function recordTopTap(clientY) {
      if (panel.classList.contains('open')) return;
      if (clientY > TOP_ZONE) return;
      const now = Date.now();
      tripleTimes = tripleTimes.filter((t) => now - t.time <= TRIPLE_MS);
      tripleTimes.push({ time: now, y: clientY });
      if (tripleTimes.length < 3) return;
      const ys = tripleTimes.map((t) => t.y);
      const spread = Math.max(...ys) - Math.min(...ys);
      tripleTimes = [];
      if (spread <= TRIPLE_SPREAD) openFromTop();
    }

    // 捕获阶段监听，避免页面自身 touch 处理拦截事件
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        clearHold();
        if (t) endHold(t.clientY);
      },
      { capture: true, passive: true }
    );

    document.addEventListener('touchcancel', clearHold, { capture: true, passive: true });

    document.addEventListener('mousedown', (e) => beginHold(e.clientY));
    document.addEventListener('mousemove', (e) => moveHold(e.clientY));
    document.addEventListener('mouseup', (e) => {
      clearHold();
      recordTopTap(e.clientY);
    });
  ...
  // v1.6.2：页面加载即自动打开设置面板，无需任何唤起动作；券面页不弹（避免挡住给店员看的券面）
  if (!isTicketPage()) {
    openPanel(true);
    npDebug('UI 初始化完成，面板已自动打开');
  } else {
    npDebug('券面页：面板不自动弹（右下角按钮可用）');
  }

  $('#npSplit', root).addEventListener('click', () => {
    const { l, f } = splitFullName($('#npFull', root).value);
    $('#npL', root).value = l;
    $('#npF', root).value = f;
    log(`已拆分：姓「${l}」名「${f}」`);
  });
  ...
  $('#npSubmit', root).addEventListener('click', async () => {
    ...
    try {
      ...
      const dn = `${after.last_name} ${after.first_name}`.trim();
      if (dn) {
        $('#npOverlayName', root).value = dn;
        if (!$('#npOverlayOn', root).checked) {
          $('#npOverlayOn', root).checked = true;
        }
        saveOverlayFromUI();
      }
    } catch (e) {
