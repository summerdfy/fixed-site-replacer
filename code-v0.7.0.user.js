...
// @version 0.7.0
// @description 本地替换 Microsoft 登录页、Outlook 和 Bandai Parks 页面上的显示文字。（iOS Safari Userscripts / 油猴双兼容；触发：长按 2 秒或三击顶部区域）v0.7：去掉 4 小时自动失效，替换结果锁定进浏览器本地缓存——关闭浏览器扩展也不会还原，只有用面板「清理并还原」才能恢复原样。
...
// ============================================================
// v0.7.0 变更（2026-08-30）
// 1. 删除 4 小时自动失效逻辑：保存后一直生效，直到手动清理。
// 2. 新增「浏览器本地缓存锁定」：保存时扫描该站点 localStorage 中所有匹配
//    「原文字」的缓存值，直接改写为替换后的文字，并把原值记入锁定清单
//    （LOCK_CACHE_KEY）。这样即使关闭 Userscripts 扩展（脚本不再运行），
//    站点自己从 localStorage 读取显示名时，看到的仍是改后的名字。
// 3. 新增「清理并还原」按钮：恢复全部被锁定的缓存原值 + 还原页面上已改文字，
//    这是唯一的还原途径。
// 4. 还原时只覆盖「当前值仍等于我们改过的值」的条目，站点自己更新过的新值
//    不会被误覆盖。
// ============================================================
...
(() => {
 ...
 const LOCK_CACHE_KEY = "codex.fixedSite.nameReplacer.lock.v1";
 ...
 // v0.7: 不再有 4 小时倒计时。开启后持续生效，直到「清理并还原」。
 const isReplacementActive = () => config.enabled && siteAuthorized();
 ...
 const isOwnStorageKey = (key) =>
  key === STORAGE_KEY || key === LOCK_CACHE_KEY || String(key).startsWith("codex.");

 const readLockCache = () => {
  try {
   const raw = localStorage.getItem(LOCK_CACHE_KEY);
 ...
 const writeLockCache = (entries) => {
  try {
   localStorage.setItem(LOCK_CACHE_KEY, JSON.stringify(entries));
  } catch {
   // 忽略写入失败（配额等）
  }
 };

 const patchValueByRules = (value, rules) => {
  let next = value;
  for (const rule of rules) {
   if (rule.mode === "regex") {
    try {
     next = next.replace(new RegExp(rule.original, "g"), rule.replacement);
    } catch {
     // 正则无效则跳过该规则
    }
   } else {
    next = next.split(rule.original).join(rule.replacement);
   }
  }
  return next;
 };

 // 还原被锁定的缓存：只有「当前值仍等于我们改过的值」才写回原值，
 // 避免覆盖站点自己更新的新数据。
 const restoreLocalStorageLock = () => {
  const entries = readLockCache();
  ...
  for (const entry of entries) {
   ...
   try {
    ...
    if (current === entry.patched) {
     localStorage.setItem(entry.key, entry.original);
     restored++;
    } else if (current === entry.original) {
     restored++; // 已经是原值
    }
    // 其他情况：站点自己更新过，不覆盖，直接放弃该锁
   } catch {
    // 忽略
   }
  }
 ...
 // 应用锁定：先解除旧锁，再按当前规则扫描并改写站点 localStorage 中所有匹配项。
 const applyLocalStorageLock = () => {
  // 规则/开关可能有变化，先解除旧锁（只还原仍是我们改过的值）
  restoreLocalStorageLock();

  if (!isReplacementActive()) return 0;

  const rules = config.rules.filter((r) => r.enabled && r.original && r.replacement);
  ...
  try {
   for (let i = 0; i < localStorage.length; i++) {
    ...
    if (!key || isOwnStorageKey(key)) continue;
    ...
    const patched = patchValueByRules(value, rules);
    if (patched === value) continue;

    lockEntries.push({ key, original: value, patched });
    try {
     localStorage.setItem(key, patched);
     patchedCount++;
    } catch {
     // 单个 key 写入失败（如配额）不影响其他
    }
   }
  } catch {
   // localStorage 整体不可用（隐私模式等）时忽略
  }

  if (lockEntries.length) {
   writeLockCache(lockEntries);
  }
  return patchedCount;
 };

 const getLockedCount = () => readLockCache().length;
 ...
 const updateStatusText = (message) => {
  ...
  if (!config.enabled) {
   ...
   statusNode.textContent = locked > 0
    ? `已停止实时替换；仍有 ${locked} 项锁定缓存保留（点「清理并还原」可恢复原样）。`
    : "替换功能已关闭，页面正常显示。";
   return;
  }

  statusNode.textContent = "替换已锁定到浏览器本地缓存：关闭浏览器扩展也不会还原，需点「清理并还原」恢复原样。";
 };
 ...
 grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) 112px 58px;
 }
 ...
 #${PANEL_ID}.is-advanced .codex-table-head,
 #${PANEL_ID}.is-advanced .codex-rule-row {
 grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) 104px 52px;
 }
 ...
 // 只停止实时替换，已锁定的缓存保留（还原请用「清理并还原」）
 const disableReplacement = () => {
 ...
 <button type="button" class="codex-btn primary" data-action="save">保存并锁定</button>
 <button type="button" class="codex-btn" data-action="disable">停止实时替换</button>
 <button type="button" class="codex-btn danger" data-action="restore-all">清理并还原</button>
 </div>
 ...
 <div>保存后会同时「锁定」到浏览器本地缓存：即使关闭浏览器扩展，站点从缓存读取的名字也不会变回原样；只有点「清理并还原」才会恢复。</div>
 ...
 `;
  ...
  panel.addEventListener("click", (event) => {
   ...
   if (action === "save") {
    ...
    saveConfig();
    applyReplacements();
    const lockedCount = applyLocalStorageLock();
    const ruleCount = config.rules.filter((rule) => rule.enabled && rule.original && rule.replacement).length;
    updateStatusText(
     `已应用 ${ruleCount} 条规则，并锁定 ${lockedCount} 项浏览器缓存。关闭浏览器扩展也不会还原；只有点「清理并还原」才能恢复原样。`
    );
    return;
   }
   ...
   if (action === "restore-all") {
    const restored = restoreLocalStorageLock();
    config.enabled = false;
    saveConfig();
    restoreTouchedContent();
    applyFontAdjust();
    updateStatusText(`✅ 已清理锁定缓存并还原 ${restored} 项，页面文字已恢复原样。`);
    return;
   }
  });
 ...
 const boot = () => {
  ...
  const mountUi = () => {
   ...
   buildPanel();
   applyReplacements();
   // 重新上锁：防止站点在脚本未运行期间把被锁定的缓存值覆盖回原样
   applyLocalStorageLock();
   startFastScanLoop();
  };
